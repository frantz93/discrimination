...
```{r acm1, out.width="75%", fig.align='center', fig.cap = "Représentation des variables quali. supp. dans le plan ACM"}
#Multicomponent analysis - supplementary qualitative variables
...
```{r acm2, out.width="75%", fig.align='center', fig.cap = "Représentation des principales variables dans le plan ACM"}
#Multicomponent analysis - main variables
...
tab3 <- tot[,] %>% flextable() %>% set_caption("Résultats de la regression OLS du logarithme du salaire horaire") %>% colformat_double(digits = 4)
#tab3
...
```{r fairlieTab}
#Decomposition de l'écart (natif - immigrant) de prob chomage (methode Fairlie)
#1.2Tableau de l'ensemble des résultats de la régression 

tab6 <- tot3[-1,] %>% flextable() %>% set_caption("Différences de caractéristiques entre natifs (A) et immigrants (B)") %>% colformat_double(digits = 4)
#tab6
...
```{r ecartChomGraph, fig.cap="Décomposition de l'écart de probabilité de chômage entre natifs et immigrants"}
...
tot5 <- cbind(tot3[-c(1:2),], estim_coef=tot2[-6,]$estim_coef, p_value=tot2[-6,]$p_value, 
              sign=tot2[-6,]$sign)
#Decompose the unemployment rate gap betwenn immigrants and canadian born citizens - table2

tab8 <- tot5[,] %>% flextable() %>% set_caption("Influence des caractéristiques individuelles (natifs(A) vs immigrants(B)) sur les probabilités de chômage") %>% colformat_double(digits = 4) %>% flextable::footnote(i=1, j=7, ref_symbols = "a", part = "header", value = as_paragraph("(***) : p-val. < 0.001 ; (**) : p-val. < 0.01; (.) : p-val. < 0.1"))
#tab8
